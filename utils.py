...
def face_landmark(img):
    ...
    if isinstance(result, dict):
        for key in result.keys():
            ...
            cv2.rectangle(img, (facial_area[0], facial_area[1]), (facial_area[2], facial_area[3]), (255, 255, 255), 3)
            cv2.circle(img, right_eye_cor, 5, (255, 0, 255), 3)
            cv2.circle(img, left_eye_cor, 5, (255, 0, 255), 3)
            cv2.circle(img, (nose_cor), 5, (0, 255, 0), 3)
            cv2.circle(img, mouth_right, 5, (0, 255, 255), 3)
            cv2.circle(img, mouth_left, 5, (0, 255, 255), 3)
...
def compare_face(img1, img2, model="VGG-Face", det_back="mtcnn"):
    ...
    faces.
        ...
        - model_name: The name
    """
    return DeepFace.verify(img1, img2, model_name=model, detector_backend=det_back, prog_bar=True)


def analyze_image(img_name):
    """
    It takes an image name as input and returns a dictionary of the image's age, gender, race, and
    emotion
    
    :param img_name: The name of the image you want to analyze
    :return: A dictionary with the keys: age, gender, race, and emotion.
    """
    return DeepFace.analyze(
        img_path = img_name, actions = ['age', 'gender', 'race', 'emotion']
        )
